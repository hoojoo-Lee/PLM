...
class Customer(Base):
    ...
    background_info = Column(Text, nullable=True, comment="客户背景信息")
    customer_team = Column(String(500), nullable=True, comment="客户团队")
    pm_contacts = Column(JSON, nullable=True, comment="PM的对接人")
    contact_email = Column(String(200), nullable=True, comment="对接邮箱")
    shipping_address = Column(Text, nullable=True, comment="发货地址")
    status = Column(String(20), server_default=text("'active'"), comment="合作状态")
    created_at = Column(DateTime(timezone=True), server_default=text("now()"))
